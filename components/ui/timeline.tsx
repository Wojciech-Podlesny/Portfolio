            ...
            <div ref={ref} className="relative max-w-7xl mx-auto pb-32">
                {data.map((item, index) => (
                    ...
                    >
                        {/* LEFT SIDE */}
                        <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                            <div className="timeline-icon h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                                <Image
                                    src={item.icon}
                                    alt={item.title}
                                    width={40}
                            ...
                            <div className="md:pl-20 text-center md:text-left">
                                <p className="hidden md:block text-gray-700 dark:text-blue-100 text-xl mt-2">
                                    {item.date}
                                </p>
                                <p className="hidden md:block font-bold text-gray-800 dark:text-blue-100 text-xl mt-2">
                                    {item.title}
                                </p>
                            </div>
                        </div>
                        ...
                        {/* RIGHT SIDE */}
                        <div className="relative pl-20 pr-4 md:pl-4 w-full">
                            <h3 className="md:hidden block text-2xl mb-2 font-bold text-black dark:text-white">
                                {item.title}
                            </h3>
                            <p className="md:hidden text-gray-700 dark:text-blue-100 text-sm mb-4">
                                {item.date}
                            </p>
                            {item.content}
                        </div>
                ...
                <div
                    style={{ height: `${height}px`, top: `${lineOffset}px` }}
                    className="absolute left-8 w-[2px] bg-gray-300 dark:bg-neutral-700"
                >
                    <motion.div
                        style={{ height: heightTransform, opacity: opacityTransform }}
                        className="absolute top-0 left-0 w-[2px] bg-gradient-to-b from-purple-500 via-blue-500 to-transparent rounded-full"
